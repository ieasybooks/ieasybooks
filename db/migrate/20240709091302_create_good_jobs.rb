...
class CreateGoodJobs < ActiveRecord::Migration[7.1]
  def change # rubocop:disable Metrics/AbcSize,Metrics/MethodLength
    ...
    add_index :good_jobs, %i[priority created_at], order: { priority: 'ASC NULLS LAST', created_at: :asc },
                                                   where: 'finished_at IS NULL', name: :index_good_job_jobs_for_candidate_lookup # rubocop:disable Metrics/LineLength
    ...
    add_index :good_jobs, %i[priority scheduled_at], order: { priority: 'ASC NULLS LAST', scheduled_at: :asc },
                                                     where: 'finished_at IS NULL AND locked_by_id IS NULL', name: :index_good_jobs_on_priority_scheduled_at_unfinished_unlocked # rubocop:disable Metrics/LineLength
